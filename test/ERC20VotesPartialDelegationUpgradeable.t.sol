...
contract PartialDelegationTest is Test {
  ...
  function assertCorrectVotes(PartialDelegation[] memory _delegations, uint256 _amount, address _delegator) internal {
    (DelegationAdjustment[] memory _votes, uint256 _remainder) =
      tokenProxy.exposed_calculateWeightDistribution(_delegations, _amount);
    uint256 _totalWeight = 0;
    for (uint256 i = 0; i < _delegations.length; i++) {
      uint256 _expectedVoteWeight =
        _delegations[i]._delegatee == _delegator ? _votes[i]._amount + _remainder : _votes[i]._amount;
      assertEq(
    ...
    assertEq(_totalWeight + _remainder, _amount, "incorrect total weight");
  }
...
contract Delegate is PartialDelegationTest {
  function testFuzz_DelegatesToSingleAddressSelf(address _actor, uint96 _numerator, uint256 _amount) public {
    ...
    tokenProxy.mint(_amount);
    PartialDelegation[] memory delegations = new PartialDelegation[](1);
    delegations[0] = PartialDelegation(_actor, _numerator);
  ...
  {
    vm.assume(_actor != address(0));
    vm.assume(_delegatee != address(0) && _delegatee != _actor);
    _numerator = uint96(bound(_numerator, 1, tokenProxy.DENOMINATOR()));
    _amount = bound(_amount, 0, type(uint208).max);
    vm.startPrank(_actor);
    tokenProxy.mint(_amount);
    ...
    vm.stopPrank();
    assertEq(tokenProxy.delegates(_actor), delegations);
    (DelegationAdjustment[] memory adjustments, uint256 _leftover) =
      tokenProxy.exposed_calculateWeightDistribution(delegations, _amount);
    assertEq(tokenProxy.getVotes(_delegatee), adjustments[0]._amount);
    assertEq(tokenProxy.getVotes(_actor), _leftover);
  }
  ...
  function testFuzz_DelegatesToTwoAddressesWithCorrectNumeratorSum(
    ...
    uint96 _numerator2 = tokenProxy.DENOMINATOR() - _numerator1;
    vm.startPrank(_actor);
    tokenProxy.mint(_amount);
    ...
    vm.stopPrank();
    assertEq(tokenProxy.delegates(_actor), delegations);
    assertCorrectVotes(delegations, _amount, _actor);
  }
  ...
  function testFuzz_DelegatesToNAddressesAndThenDelegatesToOtherAddresses(
    ...
    assertCorrectVotes(newDelegations, _amount, _actor);
...
contract Transfer is PartialDelegationTest {
  ...
  function testFuzz_HandlesTransfersToSelf(address _holder, uint256 _transferAmount, uint256 _existingBalance) public {
    ...
    vm.stopPrank();
    assertCorrectVotes(_delegations, _existingBalance, _holder);
...
contract Permit is PartialDelegationTest {
  ...
  function testFuzz_TransferWithPermit(
    uint256 _holderSeed,
    address _receiver,
  ...
  function testFuzz_RevertIf_InvalidReceiverTriesToUseAValidSignature(
    uint256 _holderSeed,
    address _receiver,
    address _invalidReceiver,
    uint256 _transferAmount,
    uint256 _existingBalance,
    uint256 _deadline
  ) public {
    ...
    vm.assume(_holder != address(0));
    vm.assume(_receiver != address(0) && _receiver != _holder);
    vm.assume(_invalidReceiver != address(0) && _invalidReceiver != _receiver && _invalidReceiver != _holder);

    _transferAmount = bound(_transferAmount, 1, type(uint208).max);
    _existingBalance = bound(_existingBalance, _transferAmount, type(uint208).max);
    ...
    vm.prank(_invalidReceiver);
    vm.expectRevert(
      abi.encodeWithSelector(ERC20InsufficientAllowance.selector, _invalidReceiver, 0, _transferAmount)
    );
    tokenProxy.transferFrom(_holder, _receiver, _transferAmount);
  }

  function testFuzz_RevertIf_TransferMoreThanTheAmountPermitted(
    uint256 _holderSeed,
    address _receiver,
    uint256 _transferAmount,
    uint256 _invalidTransferAmount,
    uint256 _existingBalance,
    uint256 _deadline
  ) public {
    _holderSeed = bound(
    ...
    vm.assume(_receiver != address(0) && _receiver != _holder);
    _transferAmount = bound(_transferAmount, 0, type(uint208).max);
    _invalidTransferAmount = bound(_invalidTransferAmount, _transferAmount + 1, type(uint256).max);
    _existingBalance = bound(_existingBalance, _transferAmount, type(uint208).max);
    ...
    vm.prank(_receiver);
    vm.expectRevert(
      abi.encodeWithSelector(ERC20InsufficientAllowance.selector, _receiver, _transferAmount, _invalidTransferAmount)
    );
    tokenProxy.transferFrom(_holder, _receiver, _invalidTransferAmount);
  }
}
